"""Phase 3D - Exact-return StockGRU training script.

Trains the GRU model on a specified feature-set NPZ file with early stopping
on the validation split and saves all history, checkpoints, metrics, and
prediction records.

Usage:
    uv run python src/financial_forecasting/train/train_exact_gru.py
    uv run python src/financial_forecasting/train/train_exact_gru.py --feature-set auxiliary_ohlc
"""
...
def main() -> None:
    """Orchestrate training of Exact-return StockGRU."""
    parser = argparse.ArgumentParser(
        description="Train exact-return StockGRU."
    )
    parser.add_argument(
        "--feature-set",
        type=str,
    ...
    print("=" * 60)
    print(f"Phase 3D - Exact-return StockGRU Training [{feature_set}]")
    print("=" * 60)

    paths = ProjectPaths()
    paths.ensure_all()

    model_params = ExactGRUModelParams()
    train_params = ExactGRUTrainingParams()
    ...
    def _resolve_npz(split: str) -> object:
        ...
        if canonical.exists():
            ...
            raise FileNotFoundError(
                f"NPZ file not found: {canonical}\n"
    ...
    run_id = (
        f"{timestamp}_exact_stock_gru_{feature_set}_seed{train_params.seed}"
    )
    ...
    print(f"StockGRU initialized on device: {device}")
...
"""
    ...
    print("\n" + "=" * 60)
    print(f"Phase 3D GRU training complete! Run ID: {run_id}")
