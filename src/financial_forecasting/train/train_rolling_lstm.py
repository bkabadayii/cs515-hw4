"""Phase 4 training script for rolling-average StockLSTM.

Trains the LSTM model on rolling-average return targets with early stopping
on the validation split and saves all history, checkpoints, metrics, and
prediction records.

Usage:
    uv run python src/financial_forecasting/train/train_rolling_lstm.py
    uv run python src/financial_forecasting/train/train_rolling_lstm.py --feature-set auxiliary_ohlc
"""
...
from financial_forecasting.parameters.rolling_lstm_params import (
    RollingLSTMModelParams,
    RollingLSTMTrainingParams,
)
...
def main() -> None:
    """Orchestrate training of rolling-average StockLSTM."""
    parser = argparse.ArgumentParser(
        description="Train rolling-average StockLSTM."
    )
    parser.add_argument(
        "--feature-set",
        type=str,
    ...
    print("=" * 60)
    print(f"Phase 4 - Rolling-Average StockLSTM Training [{feature_set}]")
    print("=" * 60)

    paths = ProjectPaths()
    paths.ensure_all()

    model_params = RollingLSTMModelParams()
    train_params = RollingLSTMTrainingParams()
    ...
    run_id = f"{timestamp}_rolling_stock_lstm_{feature_set}_seed{train_params.seed}"
    ...
    print(f"StockLSTM on device: {device}")
...
"""
    ...
    print("\n" + "=" * 60)
    print(f"Phase 4 rolling LSTM training complete! Run ID: {run_id}")
