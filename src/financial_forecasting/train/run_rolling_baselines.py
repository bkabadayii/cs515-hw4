"""Phase 4B - Run all rolling-average regression baselines.

Evaluates four non-neural baseline predictors against rolling-average
train/val/test windows and saves results in run-folder format compatible
with the aggregate analysis pipeline.
...
Usage:
    uv run python src/financial_forecasting/train/run_rolling_baselines.py
"""
...
def _get_baseline_definition(tag: str) -> str:
    ...
    defs = {
        ...
        "train_mean": (
            "Predicts the per-horizon mean rolling-average return from the training split. "
            "Tests whether the neural model beats the global conditional mean."
        ),
        "per_stock_train_mean": (
            "Predicts the per-ticker, per-horizon mean rolling-average return from the training split. "
...
def run_one_rolling_baseline(
    ...
    """Fit and evaluate one rolling baseline, save run folder, return run_id.
    ...
    tag:
        Short tag used in run_id and definition lookup.
    predictor:
        Baseline predictor instance.
    X_train, y_train, tickers_train:
        Training split arrays and metadata.
    X_val, y_val, tickers_val:
        Validation split arrays and metadata.
    X_test, y_test, tickers_test:
        Test split arrays and metadata.
    dates_train, dates_val, dates_test:
        Anchor date lists for each split.
    paths:
        Project path container.
    split_params:
        Chronological split date boundaries.
    target_params:
        Rolling target parameters.
    raw_ohlc_path:
        Path to raw OHLC CSV for reconstructing target dates.
    ...
    run_id = f"{timestamp}_rolling_{tag}_baseline"
...
### Evaluation Results (Rolling-Average Targets)
...
def main() -> None:
    """Orchestrate all four rolling-average baseline evaluations."""
    print("=" * 60)
    print("Phase 4B - Rolling-Average Regression Baselines")
    print("=" * 60)
    ...
    # Load rolling original_ohlc splits
    def _load_npz(split: str) -> StockDataset:
        npz_path = paths.data_splits / f"rolling_original_ohlc_{split}.npz"
        if not npz_path.exists():
            raise FileNotFoundError(
                f"Rolling NPZ not found: {npz_path}\n"
                "Run build_rolling_return_windows.py first."
            )
        return StockDataset(npz_path)

    print("\nLoading rolling_original_ohlc split datasets...")
    ...
    for name, tag, predictor in baselines:
        run_id = run_one_rolling_baseline(
            name=name,
            tag=tag,
            predictor=predictor,
            X_train=X_train,
    ...
    baseline_registry = paths.results_aggregate / "rolling_baseline_run_ids.json"
    with open(baseline_registry, "w", encoding="utf-8") as f:
        json.dump({"rolling_baseline_run_ids": run_ids}, f, indent=2)
    print(f"\nRolling baseline run IDs saved to: {baseline_registry}")

    print("\n" + "=" * 60)
    print("Phase 4B rolling baselines complete!")
    for rid in run_ids:
